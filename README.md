...
```json
{
  ...
  "require": {
    "craftcms/feed-me": "dev-bramble/4.x-exportable-imports"
  }
}
```
...
Use `dev-bramble/4.x-exportable-imports` for Craft 3 projects. Once a Bramble release tag exists, prefer the matching numeric patch tag, for example `4.8.1.1`, so production installs are pinned to an immutable release.
