...
class Exports extends Component
{
     ...
     * @throws Throwable
     */
    private function mapField(FeedModel $feed, mixed $source, array &$row, string $fieldHandle, array $fieldInfo, string $format): void
    {
     ...
     * @throws Throwable
     */
    private function mapNestedFields(FeedModel $feed, mixed $source, array &$row, mixed $fields, string $format): void
    {
     ...
     * @throws Throwable
     */
    private function mapBlocks(FeedModel $feed, mixed $source, array &$row, mixed $blocks, string $format): void
    {
        ...
        $items = array_filter(
            $this->valueToArray($source),
            fn(mixed $item): bool => $item instanceof CraftElementInterface,
        );
        ...
        foreach ($blocks as $blockHandle => $blockInfo) {
            ...
            $blockTypes = array_filter(array_map(
                fn(CraftElementInterface $item): ?string => $this->getBlockTypeHandle($item),
                $items,
            ));

            $blockItems = array_values(array_filter(
                $items,
                fn(CraftElementInterface $item): bool => $this->getBlockTypeHandle($item) === (string)$blockHandle,
            ));
    ...
    private function readMappedValue(mixed $source, string $fieldHandle, array $fieldInfo): mixed
    {
        if (is_array($source)) {
    ...
    private function readFieldValue(CraftElementInterface $element, string $fieldHandle): mixed
    {
        try {
    ...
    private function normalizeMappedValue(mixed $value, array $fieldInfo, FeedModel $feed, string $format): mixed
    {
        ...
        if ($elements !== null) {
            ...
            $value = array_map(
                fn(CraftElementInterface $element): mixed => $this->getElementMatchValue($element, $match),
                $elements,
            );
    ...
    private function normalizeValue(mixed $value, FeedModel $feed, bool $forCsv = false): mixed
    {
        ...
        if (is_array($value)) {
            ...
            if ($forCsv) {
                if ($this->isScalarArray($normalized)) {
                    return implode(Plugin::$plugin->service->getConfig('dataDelimiter', $feed->id), array_map(
                        fn(mixed $item): string => $this->stringifyScalar($item),
                        $normalized,
                    ));
                }
    ...
    private function getElementMatchValue(CraftElementInterface $element, string $match): mixed
    {
        if ($match === 'id') {
            return $element->id;
        }
    ...
    private function extractElements(mixed $value): ?array
    {
        if ($value instanceof CraftElementInterface) {
            return [$value];
        }
    ...
    private function setMappedValue(array &$row, string $node, mixed $value, string $format): void
    {
        if ($format === 'csv') {
            $row[$node] = $value;
            return;
        ...
        $segments = array_values(array_filter(explode('/', $node), fn(string $segment): bool => $segment !== ''));
    ...
    private function appendXmlValue(DOMDocument $document, DOMElement $parent, string $name, mixed $value): void
    {
    ...
    private function isMappableNode(mixed $node): bool
    {
        return is_string($node) && $node !== '' && !in_array($node, ['noimport', 'usedefault'], true);
    }

    private function isReadableMapping(mixed $source, array $fieldInfo): bool
    {
